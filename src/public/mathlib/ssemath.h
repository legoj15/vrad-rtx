...
extern const ALIGN16 uint32
    g_SIMD_ComponentMask[4][4] ALIGN16_POST; // [0xFFFFFFFF 0 0 0], [0
...
FORCEINLINE int
TestSignSIMD(const fltx4 &a) // mask of which floats have the high bit set
{
  int nRet = 0;
...
FORCEINLINE fltx4 SelectSIMD(const fltx4 &ReplacementMask,
                             const fltx4 &NewValue, const fltx4 &OldValue) {
  return XorSIMD(AndSIMD(XorSIMD(NewValue, OldValue), ReplacementMask),
                 OldValue);
}
    ...
    const fltx4 &a) // (a.x < 0) || (a.y < 0) || (a.z < 0) || (a.w < 0)
{
  ...
  unsigned int equalFlags = 0;
  fltx4 signMask =
      __vspltisw(-1); // 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF 0xFFFFFFFF (low order
                      // 5 bits of each element = 31)
  signMask =
      __vslw(signMask, signMask); // 0x80000000 0x80000000 0x80000000 0x80000000
...
template <typename T> FORCEINLINE fltx4 LoadAlignedSIMD(const T *pSIMD) {
  static_assert(IS_FLOAT_TYPE(T), "You are doing something wrong.");
  return _mm_load_ps(reinterpret_cast<const float *>(pSIMD));
}

template <typename T> FORCEINLINE shortx8 LoadAlignedShortSIMD(const T *pSIMD) {
  static_assert(IS_FLOAT_TYPE(T), "You are doing something wrong.");
...
public:
  ...
  FORCEINLINE void
  operator+=(FourVectors const &b) //< add 4 vectors to another 4 vectors
  {
    x = AddSIMD(x, b.x);
    y = AddSIMD(y, b.y);
    z = AddSIMD(z, b.z);
  ...
  FORCEINLINE void
  operator-=(FourVectors const &b) //< subtract 4 vectors from another 4
  {
    x = SubSIMD(x, b.x);
    y = SubSIMD(y, b.y);
    z = SubSIMD(z, b.z);
  ...
  FORCEINLINE void
  MakeReciprocalSaturate(void) //< (x,y,z)=(1/x,1/y,1/z), 1/0=1.0e23
  {
