...
//function. 			ppData - Receives the context data for the given chunk.
...
//value of the key. 			nValueSize - Size of the buffer pointed to by szValue.
...
ChunkFileResult_t CChunkFile::WriteLine(const char *pszLine) {
  if (pszLine != NULL) {
    ...
    //
    if (m_nCurrentDepth > 0) {
      int nWritten = (int)fwrite(m_szIndent, 1, m_nCurrentDepth, m_hFile);
    ...
    int nWritten = (int)fwrite(pszLine, 1, nLen, m_hFile);
    if (nWritten != nLen) {
      return (ChunkFile_Fail);
    }
