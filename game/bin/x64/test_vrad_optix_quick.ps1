...
$MOD_DIR = "..\..\mod_hl2mp"
$LOG_FILE = "test_vrad_optix_quick.log"
$MAP_NAME = "validation_vrad_quick"
...
try {
    ...
    # Skip recompilation if neither the source BSP nor this script have changed.
    $SCRIPT_PATH = $MyInvocation.MyCommand.Path
    $REF_HASH_FILE = "$REF_DIR\$MAP_NAME.ref_hash"
    $currentHash = (Get-FileHash $MAP_SRC -Algorithm SHA256).Hash + ":" + (Get-FileHash $SCRIPT_PATH -Algorithm SHA256).Hash
...
$RUN_DIR = "$ARCHIVE_DIR\${timestamp}_quick"
New-Item -ItemType Directory -Path $RUN_DIR | Out-Null

$logMap = @{
    $LOG_FILE    = "test_quick.log"
